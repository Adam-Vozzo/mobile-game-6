...
- Branch: `claude/elegant-lamport-Nvnn3` (iter 1 — SpringArm camera + profiles).
- Focus: SpringArm3D collision avoidance shipped; Floaty + Momentum profiles
  added; camera params section in dev menu. See README for full entry.
...
1. **On-device smoke test.** Open the project in Godot 4.6 Mobile, fix
   ...
   _Blocked: needs human with Godot 4.6 + test device._
2. **Tune Snappy on device.** Adjust `resources/profiles/snappy.tres`
   gravity / jump_velocity / accel / coyote / buffer based on first
   on-device feel. Compare against Floaty and Momentum.
   _Blocked: needs first on-device feel._
3. **Author Assisted profile (`assisted.tres`)** — in-air steering toward
   likely landing target, generous ledge grab, edge-snap on landing.
   _Requires player.gd changes (ledge assist, edge snap); scope discussion
   before starting._
4. **Touch overlay polish.** Drag-to-place reposition mode invoked from
   the dev menu (handles per control, snap-to-thumb-zone presets,
   resize on jump button). Persist anchors + radii to `user://input.cfg`.
5. **Dev menu fleshing.** Debug-viz toggles (collision shapes, velocity
...
7. **Soft distance lerp when spring arm shortens.** Currently the camera
   jumps to the shortened distance instantly. Add a lerp so the pull-in
   is smooth. See `camera_mobile_3d.md` research note.
8. **Momentum profile — add Curve field to ControllerProfile** for
   non-linear speed ramp (currently ships as linear approximation).
   Requires player.gd + new curve resource. See DECISIONS.md.

### Refactor backlog

- `dev_menu_overlay.gd::_build_ui` is ~100 lines. Split into
  `_build_controller_section`, `_build_camera_section`, `_build_juice_section`
  helper methods.
...
- 2026-05-08 (iter 1) — SpringArm3D collision avoidance on camera rig;
  Floaty + Momentum profiles; camera params section in dev menu (distance,
  pitch, lookahead, fall-pull, yaw-sens sliders); camera research note
  (`docs/research/camera_mobile_3d.md`). See README.
