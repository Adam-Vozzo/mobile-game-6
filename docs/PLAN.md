...
- Branch: `claude/elegant-lamport-Ghme0`
- Focus: SpringArm3D camera collision + Floaty/Momentum profiles done.
  Next: on-device test (human-gated), then camera params dev menu group.
...
1. **On-device smoke test.** Open the project in Godot 4.6 Mobile, fix
   ...
   _(Human-gated: needs Godot binary + test device.)_
2. **Tune Snappy on device.** Adjust `resources/profiles/snappy.tres`
   gravity / jump_velocity / accel / coyote / buffer based on first
   on-device feel. Floaty and Momentum profiles exist — compare all three
   once the build runs.
   _(Human-gated: requires first feel verdict.)_
...
4. **Camera params group in dev menu.** Wire sliders to the rig's
...
6. **Dev menu fleshing.** Debug-viz toggles (collision shapes, velocity
...
- 2026-05-09 — SpringArm3D camera collision avoidance (sphere cast,
  fast pull-in / slow recovery lerp, manual camera placement). Floaty
  and Momentum profiles authored. Dev menu dropdown expanded to 3 profiles.
  See README's Updates section (2026-05-09 entry).
