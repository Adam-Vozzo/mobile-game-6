...
2. **Tune Snappy on device.** Adjust `resources/profiles/snappy.tres`
   ...
   _(Floaty also now available for comparison — see side quest below.)_
3. **Author Momentum profile (`momentum.tres`)** with sustained-input
   speed ramp. Add a sliders-affected curve for the ramp.
4. **Author Assisted profile (`assisted.tres`)** — in-air steering toward
   likely landing target, generous ledge grab, edge-snap on landing.
5. **Camera params group in dev menu.** Wire sliders to the rig's
...
- 2026-05-09 — Iteration 1: SpringArm3D camera collision avoidance
  (camera_rig.tscn + camera_rig.gd rewritten; player excluded from
  SpringArm collision; math verified). Side quest: Floaty profile
  authored (floaty.tres, dadish-leaning values) and registered in
  dev_menu_overlay.gd. Branch: `claude/elegant-lamport-M4XQE`.
